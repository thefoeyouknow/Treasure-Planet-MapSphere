...
BLECharacteristic configChar("a1b2c3d4-e5f6-7a8b-9c0d-1e2f3a4b5c70", BLERead | BLEWrite, 3);
...
void initBLE() {
  ...
  uint8_t initialConfig[3] = {35, 255, 50}; // 35s, max brightness, default offset
  configChar.writeValue(initialConfig, 3);
  ...
  if (isScanning) {
    ...
    if (peripheral) {
      ...
      if (proximityActive && filteredRSSI < proxSleepGate) {
        ...
        Serial.println("[LED] Mode: AUTONOMOUS (Searching)");
      }
    }
  }
